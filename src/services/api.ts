...
const API_BASE_URL = process.env.EXPO_PUBLIC_API_URL || 'http://192.168.1.3:3000/api';
...
class ApiService {
  ...
  async searchPrompts(query: string, page = 1): Promise<ApiResponse<{ prompts: Prompt[]; total: number }>> {
    return this.request(`/prompts/search?q=${encodeURIComponent(query)}&page=${page}`);
  }
  ...
  async reactToPrompt(promptId: string, reaction: string): Promise<ApiResponse<void>> {
  ...
  async submitFeedback(promptId: string, feedback: {
    usefulness: number;
  ...
  async getSavedPrompts(page = 1): Promise<ApiResponse<{ prompts: Prompt[]; total: number }>> {
    return this.request(`/user/saved?page=${page}`);
  }
  
  // ============ NOTIFICATIONS ============
  
  async getNotifications(page = 1): Promise<ApiResponse<{ notifications: any[]; unread: number }>> {
  ...
  async getChartData(type: 'platform' | 'aiTool' | 'category', period: 'day' | 'week' | 'month'): Promise<ApiResponse<any>> {
